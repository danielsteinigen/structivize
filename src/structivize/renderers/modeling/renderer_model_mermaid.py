...
class RendererModelMermaid(Renderer):
    DEFAULT_TOOL_CONFIGS = {"mermaid": {"theme": "default", "background": "white"}}
    FILE_EXT = "mmd"
    ...
    def _render_mermaid(self):
        self._execute_process(
            commands=[
                ...
                "3",
                "--theme",
                self.tool_config.get("theme", "default"),
                "--backgroundColor",
                f"{'transparent' if self._image_transparent else self.tool_config.get('background', 'white')}",
                "-o",
