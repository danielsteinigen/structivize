...
class RendererBioVienna(Renderer):
    ...
    def preprocess_code(self) -> str:
        ...
        for line in self._code.splitlines():
            line_str = line.strip()
            if not line_str.startswith("#") and not line_str.startswith("!") and not line_str.startswith(";") and not line_str.startswith("//") and line_str != "":
                code += f"{line}\n"
        self._code = code.strip()
        if '>' == self._code[0] and len(self._code.splitlines()) == 2 and '(' in self._code.splitlines()[1] and 'A' in self._code.splitlines()[0]:
    ...
    def _render_varna(self):
        ...
        self._execute_process(commands=["java", "-cp", f"{self._tool_path}/VARNAv3-93.jar", "fr.orsay.lri.varna.applications.VARNAcmd", "-i", self._filepath_code, "-o", f"{self.filepath_image}.png", "-titleSize", "0", "-resolution", "2.0", "-zoom", "1.0", "-border", "'20x30'", "-algorithm", "radiate"]) #  -algorithm naview
    ...
    def _render_r2dt(self):
        ...
        self._execute_process(commands=["cp", self._filepath_code, f"{path_temp}/test.fasta"])
        ...
        os.system(f"docker run --entrypoint r2dt.py -v $R2DT_LIBRARY:/rna/r2dt/data/cms -v {path_temp}:/rna/r2dt/temp rnacentral/r2dt draw temp/test.fasta temp/test")
        svg_files = glob.glob(f"{path_temp}/test/results/svg/*.svg")
        self._execute_process(commands=["cp", svg_files[0], f"{self.filepath_image}.svg"])
